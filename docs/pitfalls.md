...
本协议（无菌室编程 + 精确导入 + 零 / +  显式化）适用于以下文件/模块：

| 层级 | 适用模块 | 说明 |
|------|----------|------|
| **执行层** | 、、 | 内存/IO 操作，必须零未定义行为 |
| **路由层** | 、 | 类型安全路由，禁止  隐式转换 |
| **观测层** | 、 | 事件发射，禁止阻塞 |
| **存储层** | 、 | 持久化，错误必须显式处理 |
...
|  及  | 测试代码，允许便利写法 |
|  | 程序入口，允许一次性  |
| C 交互层（、 等  用户） | 必须链接 libc，允许  导入 |

### 违规处理流程

1. **P0 违规**（无菌室污染、未定义行为）：立即修复，禁止合并
2. **P1 违规**（精确导入缺失、/ 滥用）：下一版本修复
3. **P2 违规**（文档缺失、测试覆盖不足）：记录技术债，v3.1+ 修复

> **注意**： 是系统大脑，任何 P0 违规都是**安全漏洞**，必须零容忍。
...
本协议（无菌室编程 + 精确导入 + 零 try/catch + ?Protocol 显式化）适用于以下文件/模块：

| 层级 | 适用模块 | 说明 |
|------|----------|------|
| **执行层** | protocol.zig、reactor.zig、io_uring.zig | 内存/IO 操作，必须零未定义行为 |
| **路由层** | router.zig、comptime_router.zig | 类型安全路由，禁止 anyopaque 隐式转换 |
| **观测层** | ibus.zig、feedback.zig | 事件发射，禁止阻塞 |
| **存储层** | storage.zig、file_store.zig | 持久化，错误必须显式处理 |

### 不适用范围（允许 try/catch 或 std 整体导入）

| 模块 | 原因 |
|------|------|
| tests.zig 及 integration_p*.zig | 测试代码，允许便利写法 |
| main.zig | 程序入口，允许一次性 try |
| C 交互层（context.zig、http_log.zig 等 @cImport 用户） | 必须链接 libc，允许 std 导入 |

### 违规处理流程

1. **P0 违规**（无菌室污染、未定义行为）：立即修复，禁止合并
2. **P1 违规**（精确导入缺失、try/catch 滥用）：下一版本修复
3. **P2 违规**（文档缺失、测试覆盖不足）：记录技术债，v3.1+ 修复

> **注意**：protocol.zig 是系统大脑，任何 P0 违规都是**安全漏洞**，必须零容忍。
