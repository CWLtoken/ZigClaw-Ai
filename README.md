...
> **当前状态：v6.9.1 — 军规合规 + CI 强化**  
> 测试状态：**153/153 全绿** ✅（ReleaseSafe）| 标签：`v6.9.1-milestone-all-issues-fixed`
|
---
...
| **🔒 编译期契约验证** | ContractVerifier 完整签名检查（返回类型+参数类型） |
| **🧪 153/153 全绿** | 覆盖六层架构全链路 + 集成测试 + 军规合规 |
|
---
...
| `ibus: formatBinaryMetrics 二进制帧` | 20 指标帧流 + 字段ID顺序 | 观测层 |
...
| `interface: ContractVerifier` | 编译期签名验证通过 | 观测层 |
| `reactor: poll flush 错误处理` | catch 显式 log.warn | 执行层 |
|
---
...
`zig build test -OReleaseSafe` + `bash scripts/ci_check.sh` 全部通过方可合并。
...
| v6.9.0 | v6.9.0-f2f3 | F2+F3 前瞻 | 二进制指标协议（formatBinaryMetrics）+ 更严格 comptime 合约（ContractVerifier 签名验证）| 148 |
| v6.9.1 | v6.9.1-milestone | 全局修复 | 军规合规（整包导入/catch-empty）+ CI 强化（ci_check.sh 接入）+ 文档整理 | 153 |
...
| P2-4: 二进制指标协议 | ibus.zig（formatBinaryMetrics）+ metrics.zig | ✅ v6.9.0 |
|
### 后续计划（v3.1+ 维护版本）
...
| 二进制指标/日志直写 | 观测层 | P2-4，formatBinaryMetrics 已就位，sidecar 待部署 | ✅ v6.9.0 |
...
2. **测试驱动**：新功能必须附带测试，保持 153/153 全绿
