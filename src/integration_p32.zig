...
test "Phase32: 图像子脑（LCG 256维）→ 编排 → 量化 → 推理 全链路" {
    ...
    // 3. 编排器处理第一张图像（量化路径）
    @import("std").debug.print("🔄 处理图像1: {s}\n", .{image_data_1});
    _ = try orch.orchestrate(image_data_1, .Image, &seq);
    try testing.expect(seq.len >= 1);
    
    // 验证 Token 类型
    const tok1 = seq.get(0).?;
    try testing.expect(tok1.tpe == .VectorQuantized);
    @import("std").debug.print("✅ 图像1 Token 量化成功 (类型: VectorQuantized)\n", .{});
    
    // 4. 添加第二张图像
    @import("std").debug.print("🔄 处理图像2: {s}\n", .{image_data_2});
    _ = try orch.orchestrate(image_data_2, .Image, &seq);
...
test "Phase32: 多模态混合 - 文本 + 图像 + 文本 → 全链路" {
    ...
    // 添加文本
    @import("std").debug.print("\n🔄 添加文本1: {s}\n", .{text1});
    _ = try orch.orchestrate(text1, .Text, &seq);
    
    // 添加图像
    @import("std").debug.print("🔄 添加图像: {s}\n", .{image_data});
    _ = try orch.orchestrate(image_data, .Image, &seq);
    
    // 再添加文本
    @import("std").debug.print("🔄 添加文本2: {s}\n", .{text2});
    _ = try orch.orchestrate(text2, .Text, &seq);
    
    // 3. 验证序列
    try testing.expectEqual(@as(u16, 3), seq.len);
    @import("std").debug.print("✅ Token序列长度: {}\n", .{seq.len});
...
test "Phase32: 图像子脑维度验证（64维）" {
    ...
    @import("std").debug.print("✅ 图像子脑输出64维向量，范围[-1,1]验证通过\n", .{});
}
