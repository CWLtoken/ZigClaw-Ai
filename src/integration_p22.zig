...
test "P22-S1: StreamWindow fill" {
    ...
    try testing.expectEqual(@as(u64, constants.SLOT_COUNT), window.len);

    // 尝试添加第65个（应该被静默丢弃）
    var header65 = core.TokenStreamHeader.init();
    mem.writeInt(u64, header65.data[0..8], 1064, .little);
    window.push_header(header65);
    try testing.expectEqual(@as(u64, constants.SLOT_COUNT), window.len); // 仍然是64
}

test "P22-S2: verify all slots accessible and releasable" {
    var window = storage.StreamWindow.init();
    ...
    while (i < constants.SLOT_COUNT) : (i += 1) {
        ...
        try testing.expectEqual(@as(u64, constants.SLOT_COUNT - i - 1), window.len);
    }
    try testing.expectEqual(@as(u64, 0), window.len);
}
...
test "P22-S3: DISABLED - SendDone-WaitRequest and timeout reclamation" {
