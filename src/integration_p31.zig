...
test "P31: infer_from_tokens + 编排层 + 推理层全链路 - 文本直通" {
    ...
    @import("std").debug.print("🚀 调用 infer_from_tokens...\n", .{});
    var result = try inference.infer_from_tokens(allocator, &seq, 150, "fake-key");
    defer result.deinit(allocator);

    // 6. 验证推理结果
    try testing.expect(result.len > 0);
    @import("std").debug.print("✅ P31 文本直通全链路验证成功！推理结果: {s}\n", .{result.text});
}
...
test "P31: infer_from_tokens + 图像模态（模拟）" {
    ...
    @import("std").debug.print("🚀 调用 infer_from_tokens（跳过非文本Token）...\n", .{});
    var result = try inference.infer_from_tokens(allocator, &seq, 200, "test-key");
    defer result.deinit(allocator);

    try testing.expect(result.len > 0);
    @import("std").debug.print("✅ P31 图像模态全链路验证成功！\n", .{});
}
...
test "P31: 多模态混合序列 - 文本 + 图像" {
    ...
    // 验证结果包含文本（跳过图像Token）
    try testing.expect(result.len > 0);
    @import("std").debug.print("✅ P31 多模态全链路验证成功！推理结果: {s}\n", .{result.text});
}
