...
test "P41: ServerMetrics 错误计数递增" {
    ...
    try @import("std").testing.expect(metrics.get_error_count() == 3);
}
...
test "P41: 服务器优雅关闭标志" {
    ...
    // 初始状态：正在运行
    try @import("std").testing.expect(server.is_running() == true);
    
    // 请求关闭
    server.shutdown();
    
    // 验证：已停止
    try @import("std").testing.expect(server.is_running() == false);
}
...
test "P41: 模拟推理引擎故障 - 返回 503" {
    ...
    // 验证响应包含 503
    try @import("std").testing.expect(mem.indexOf(u8, response, "503") != null);
}
