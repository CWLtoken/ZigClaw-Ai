...
test "P38: HTTP 请求解析 - GET /infer" {
    ...
    try std.testing.expect(handler.request.query != null);
    
    // 验证查询参数解析
    const query = handler.request.query.?;
    try std.testing.expect(mem.indexOf(u8, query, "input=hello") != null);
    try std.testing.expect(mem.indexOf(u8, query, "modality=text") != null);
}
...
test "P38: HTTP 请求解析 - POST /infer with body" {
    ...
    try std.testing.expect(mem.eql(u8, handler.request.method, "POST"));
    try std.testing.expect(mem.eql(u8, handler.request.path, "/infer"));
}
...
test "P38: HTTP 响应生成 - 200 OK" {
    ...
    // 调试：打印响应字符串
    std.debug.print("生成的HTTP响应:\n{s}\n", .{response_str});
    
    // 验证响应格式
    try std.testing.expect(mem.indexOf(u8, response_str, "HTTP/1.1 200 OK") != null);
    // 动态验证 Content-Length（根据实际 body 长度）
    const expected_length_str = std.fmt.allocPrint(std.heap.page_allocator, "Content-Length: {d}", .{handler.response.body.len}) catch return error.OutOfMemory;
    defer std.heap.page_allocator.free(expected_length_str);
    try std.testing.expect(mem.indexOf(u8, response_str, expected_length_str) != null);
    try std.testing.expect(mem.indexOf(u8, response_str, "{\"result\":\"success\"}") != null);
}
