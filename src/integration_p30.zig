...
test "Phase30: 编排层集成 - 文本直通 → 模拟推理" {
    ...
    @import("std").debug.print("✅ Token 生成正确: {s}\n", .{tok.getText()});

    // 5. 模拟调用推理引擎（使用 Token 文本拼接成 prompt）
    var prompt_buf: [512]u8 = undefined;
    const prompt_len = @import("std").fmt.bufPrint(prompt_buf[0..], "用户问题：{s}", .{tok.getText()}) catch "默认问题";
    
    @import("std").debug.print("🚀 调用推理引擎（模拟）...\n", .{});
    var result = try inference.infer(allocator, prompt_len, 150, "fake-key");
    defer result.deinit(allocator);

    try testing.expect(result.len > 0);
    @import("std").debug.print("✅ Phase30 集成测试成功！\n", .{});
}
...
test "Phase30: 编排层集成 - 图像模态（模拟）" {
    ...
    @import("std").debug.print("✅ 图像 Token 量化成功\n", .{});
}
...
test "Phase30: 整体链路验证 - 文本直通 + 推理 + 响应" {
    ...
    while (i < seq.len) : (i += 1) {
        ...
        if (t.tpe == .Text) {
            ...
            @import("std").mem.copyForwards(u8, full_prompt[prompt_len..], text);
            prompt_len += text.len;
        }
    }
    ...
    try testing.expect(result.len > 0);
    @import("std").debug.print("✅ Phase30 全链路验证成功！推理结果: {s}\n", .{result.text});
}
